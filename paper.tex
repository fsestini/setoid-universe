...
computer implementation, but as normally defined (for example, in \cite{mltt75})
...
Note that \textsf{transp} provides a strong elimination principle allowing to
eliminate a strict proposition (the identity type) into arbitrary types. Indeed,
the reason why we must assume this identity type as primitive is because, even
though we could have defined \textsf{Id} from $W_p$, \textsf{transp} would not
be definable from $W_p$ elimination. \todo{what is the justification for this?}

\subsection{Formalization}

A universe of strict propositions has been recently added to the Agda proof
assistant \cite{agda-prop}, thus making $MLTT^{Prop}$ a subset of Agda. In fact,
most of the developments presented here have been formalized and proof-checked
using Agda extended with strict propositions.
%
For convenience, we slightly deviate from $MLTT^{Prop}$ both in the paper and in
the formalization, for instance by relying on pattern matching instead of
...
that everything can be equivalently carried out in $MLTT^{Prop}$, although we
haven't fully checked all the necessary details.
...
\cite{hofmann}, in order to give a semantics to extensionality
principles such as function and propositional extensionality. Here we consider
a similar model construction due to Altenkirch \cite{setoid99}.
%
...
terms of a small, entirely intensional theory.
...
formers \cite{internal-tt}. The core structure of a CwF can be given as the
following signature:
...
\cite{setoid99}, but it is equivalent to it \todo{add citation}. The main
difference here is in the use of heterogeneous equality in the definition of
types.
...
of strict propositions satisfying propositional extensionality
\cite{setoid99}. Note that we don't need neither identity types nor inductive
types (W-types) in the metatheory to achieve this. However, W-types are needed
when constructing the setoid universe in the next sections.
...
rules of equality on $\Sigma$ types. Hence, Martin-L\"of's J eliminator can also
be derived \todo{add citation}.
...
model construction recalled in Section TODO and shown originally in
\cite{setoid99} is that it can be carried out in a very small type theoretic
...
definitions. We recall this reduction in Section~\ref{ir-universes}.
...
%
The question arises because we only define a weak eliminator for our final
inductive-inductive encoding of the setoid universe, for reasons that are
explained in Section~\ref{inductive-universe}. Although not currently needed, a
stronger eliminator might be necessary to justify universe induction. This
problem should not arise in the other encodings of the setoid universe (as given
in Section~\ref{ir-setoid-universe} and Section~\ref{ii-universe}).
...
Induction-recursion is arguably a nicer and more natural way to define
internal universes in Type Theory, however it is not always strictly required.
%
...
\emph{carves out} all types in $\Set$ that are in the image of $\El$. In other
...
the type of codes, the definition of $\ad{in-U}$ quite expectedly reflecs the
...
$\mType$, since the definition of $\U$ quantifies over all possible sets. Hence
...
relation reflecting equality of codes:
%
\begin{align*}
...
definition of everything else. Below we only show the more interesting
...
is particularly complex, and not obviously reducible to understood notions of
induction-recursion like in \cite{dybjer-setzer}. Nevertheless, we would like to
avoid extending the metatheory with any forms of induction-recursion, as per our
goal to keep the metatheory as small and essential as possible.

As a consequence, in the next section we transform our current
inductive-recursive definition to one that doesn't use induction-recursion.
%
...
knowledge, this is the first time this reduction trick is applied to an
inductive-recursive type of this kind.

\subsection{Inductive-inductive universe}\label{ii-universe}

We will follows the same method outlined in Section~\ref{ir-universes}. In
...
first one in their signature \cite{ii}. The main example of induction-induction
can be found in intrinsic definitions of Type Theory in Type Theory
\cite{tt-in-tt}.
...
of which the setoid universe defined in Section~\ref{ii-universe} is an
instance.
%
...
Defining the well-typing predicates in $\mProp$ does help, since otherwise we
would need function extensionality to prove that these types are propositions
(with respect to metatheoretic propositional equality).
%
...
Previous work \cite{abel2019failure} seem to suggest that the strong transport
rule for proof-irrelevant identity types is a non-trivial addition to the
metatheory.
%
We would like to investigate a construction of the setoid universe that does not
require such rule, not only to avoid unnecessary extensions to the metatheory,
but also because we don't yet know whether $MLTT^{Prop}$ extended with this rule
is normalizing.

We want to figure out whether our inductive encoding of the setoid universe can
support universe induction. A related question is whether the alternative
definitions using induction-recursion and induction-induction can support it.

SeTT \cite{mpc19} and XTT \cite{xtt} are both syntactic representations of the
setoid model with similar design choices, like definitional
proof-irrelevance. We would like to know whether their respective notions of
models are equivalent, that is, whether we can obtain an XTT model from a SeTT
model, and vice versa. Since XTT universes support universe induction, it is
possible that we would need to extend our own universe with the same principle
(see discussion in Section~\ref{setoid-model} and the previous paragraph). A
further question is whether this mapping of models is functorial.
