...
data in-U~ₜ where
  ...
  π~ₜ :
    ...
    in-U~ₜ (πₚ a₀ₚ A₀~ b₀ₚ B₀~) (πₚ a₁ₚ A₁~ b₁ₚ B₁~)
      (π~ₚ a₀ₚ a₀~ₚ a₁ₚ a₁~ₚ a₀₁ₚ b₀ₚ b₀~ₚ b₁ₚ b₁~ₚ b₀₁ₚ)

ind-in-Uₜ :
  ∀{i}{C : Prop i}
  {A : Set}{aₚ : in-Uₚ A}{A~ : A → A → Prop}{B : A → Set}{bₚ : (x : A) → in-Uₚ (B x)}{B~ : {x₀ x₁ : A}(x₀₁ : A~ x₀ x₁) → B x₀ → B x₁ → Prop} →
  ((a : in-Uₜ aₚ){a~ₚ : in-U~ₚ A~}(a~ : in-U~ₜ aₚ aₚ a~ₚ)(b : (x : A) → in-Uₜ (bₚ x))
