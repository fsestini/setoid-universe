...
module IR where
  ...
  _⊢_~El_ {π a b refb} {π a' b' refb'} w~ (f ,sp _) (f' ,sp _) = (x : El a)(x' : El a')(x~ : ↑ps (proj₁p w~ ⊢ x ~El x')) → proj₂p w~ _ _ x~ ⊢ f x ~El f' x'
  ...
  symU {π a b refb} {π a' b' refb'}(a~ ,p b~) = symU a~ ,p λ _ _ x~ → symU (b~ _ _ (mk↑ps (symEl {a~ = symU a~} (un↑ps x~))))
  symU {ι a} {bool} ()
  symU {ι a} {⊥s} ()
