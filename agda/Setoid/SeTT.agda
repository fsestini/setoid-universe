...
(Γ ~C) {Ω = Ω} σ₀ σ₁ = (γ : ∣ Ω ∣C) →  Γ C ∣ σ₀ ∣s γ ~ ∣ σ₁ ∣s γ
...
~t ((A ~T) σ₀₁ t₀ t₁) γ~ = liftP
  ((λ t₀₁ → transT3 A (symT A (~t t₀ γ~)) (unliftp t₀₁) (~t t₁ γ~)) ,p
  (λ t₀₁ → transT3 A (~t t₀ γ~) (unliftp t₀₁) (symT A (~t t₁ γ~))))

RT : ∀{i}{Γ : Con i}{j}{Ω : Con j}{k}(A : Ty Γ k){σ : Tms Ω Γ}(t : Tm Ω (A [ σ ]T)) → Tm Ω (ElP ((A ~T) {σ₀ = σ}{σ} (RC Γ σ) t t))
∣ RT A t ∣t γ = liftp (refT A (∣ t ∣t γ))
~t (RT A t) _ = liftP ttp
...
∣ ST A t₀₁ ∣t γ = liftp (symT A (unliftp (∣ t₀₁ ∣t γ)))
~t (ST A t₀₁) _ = liftP ttp
...
∣ TT A t₀₁ t₁₂ ∣t γ = liftp (transT A (unliftp (∣ t₀₁ ∣t γ)) (unliftp (∣ t₁₂ ∣t γ)))
~t (TT A t₀₁ t₁₂) _ = liftP ttp
