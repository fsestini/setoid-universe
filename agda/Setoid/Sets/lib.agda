...
data in-U~ₜ : {A₀ A₁ : Set}(a₀ : in-Uₚ A₀)(a₁ : in-Uₚ A₁){A₀₁ : A₀ → A₁ → Prop}
            → in-U~ₚ A₀₁ → Prop₁
...
inv-bool~ₜ-≡ : ∀ {x y} → in-U~ₜ x y bool~ₚ → x ≡p boolₚ ×p y ≡p boolₚ
inv-bool~ₜ-≡ bool~ₜ = reflp ,p reflp

module _
  {A} {A~ : A → A → Prop} {B : A → Set}
  {B~ : {x₀ x₁ : A} → A~ x₀ x₁ → B x₀ → B x₁ → Prop}
  {aₚ} {a~ₚ : in-U~ₚ A~} {bₚ : (x : A) → in-Uₚ (B x)}
  {b~ₚ : {x₀ x₁ : A} (x₀₁ : A~ x₀ x₁) → in-U~ₚ (B~ x₀₁)}
  where

  inv-πₜ-A : in-Uₜ (πₚ aₚ a~ₚ bₚ {B~ = B~} b~ₚ) → in-Uₜ aₚ
  inv-πₜ-A (πₜ a a~ b b~) = a

  inv-πₜ-A~ : in-Uₜ (πₚ aₚ a~ₚ bₚ {B~ = B~} b~ₚ) → in-U~ₜ aₚ aₚ a~ₚ
  inv-πₜ-A~ (πₜ a a~ b b~) = a~

  inv-πₜ-B : in-Uₜ (πₚ aₚ a~ₚ bₚ {B~ = B~} b~ₚ) → (x : _) → in-Uₜ (bₚ x)
  inv-πₜ-B (πₜ a a~ b b~) = b

  inv-πₜ-B~ : in-Uₜ (πₚ aₚ a~ₚ bₚ {B~ = B~} b~ₚ)
            → ∀{x₀ x₁} (x₀₁ : A~ x₀ x₁) → in-U~ₜ (bₚ x₀) (bₚ x₁) (b~ₚ x₀₁)
  inv-πₜ-B~ (πₜ a a~ b b~) = b~
...
module _
  ...
  T = in-U~ₜ x₀ x₁ (π~ₚ a₀ₚ a₀~ₚ a₁ₚ a₁~ₚ a₀₁ₚ b₀ₚ {B₀~} b₀~ₚ b₁ₚ {B₁~} b₁~ₚ {B₀₁} b₀₁ₚ)

  inv-π~ₜ-≡₀ : T → x₀ ≡p πₚ a₀ₚ a₀~ₚ b₀ₚ b₀~ₚ
  inv-π~ₜ-≡₀ (π~ₜ a₀ p a₁ p₁ p₂ b₀ b₀~ b₁ b₁~ b₀₁) = reflp

  inv-π~ₜ-≡₁ : T → x₁ ≡p πₚ a₁ₚ a₁~ₚ b₁ₚ b₁~ₚ
  inv-π~ₜ-≡₁ (π~ₜ a₀ p a₁ p₁ p₂ b₀ b₀~ b₁ b₁~ b₀₁) = reflp

  inv-π~ₜ-A₀ : T → in-Uₜ a₀ₚ
  inv-π~ₜ-A₀ (π~ₜ a₀ _ _ _ _ _ _ _ _ _) = a₀

  inv-π~ₜ-A₀~ : T → in-U~ₜ a₀ₚ a₀ₚ a₀~ₚ
  inv-π~ₜ-A₀~ (π~ₜ _ x _ _ _ _ _ _ _ _) = x

  inv-π~ₜ-A₁ : T → in-Uₜ a₁ₚ
  inv-π~ₜ-A₁ (π~ₜ _ _ x _ _ _ _ _ _ _) = x

  inv-π~ₜ-A₁~ : T → in-U~ₜ a₁ₚ a₁ₚ a₁~ₚ
  inv-π~ₜ-A₁~ (π~ₜ _ _ _ x _ _ _ _ _ _) = x

module simple
  {i}
  {j}
  (in-Uᴰ : ∀{A} → in-U A → Set i)
  (in-U~ᴰ : ∀{A₀ A₁ A₀₁} (a₀ : in-U A₀) (a₁ : in-U A₁) → in-U~ a₀ a₁ A₀₁ → Set j)
  (boolᴰ : in-Uᴰ bool)
  (πᴰ : ∀{A A~} {a : in-U A}(aᴰ : in-Uᴰ a) {a~ : in-U~ a a A~}(a~ᴰ : in-U~ᴰ a a a~)
    {B : A → Set}{b : (x : A) → in-U (B x)}(bᴰ : (x : A) → in-Uᴰ (b x))
    {B~ : {x₀ x₁ : A}(x₀₁ : A~ x₀ x₁) → B x₀ → B x₁ → Prop}
    {b~ : {x₀ x₁ : A}(x₀₁ : A~ x₀ x₁) → in-U~ (b x₀) (b x₁) (B~ x₀₁)}
    (b~ᴰ : {x₀ x₁ : A}(x₀₁ : A~ x₀ x₁) → in-U~ᴰ (b x₀) (b x₁) (b~ x₀₁)) → 
    in-Uᴰ (π a a~ b b~))
  (bool~ᴰ : in-U~ᴰ bool bool bool~)
  (π~ᴰ : {A₀ : Set}{a₀ : in-U A₀}(a₀ᴰ : in-Uᴰ a₀){A₀~ : A₀ → A₀ → Prop}{a₀~ : in-U~ a₀ a₀ A₀~}(a₀~ᴰ : in-U~ᴰ a₀ a₀ a₀~)
    {A₁ : Set}{a₁ : in-U A₁}(a₁ᴰ : in-Uᴰ a₁){A₁~ : A₁ → A₁ → Prop}{a₁~ : in-U~ a₁ a₁ A₁~}(a₁~ᴰ : in-U~ᴰ a₁ a₁ a₁~)
    {A₀₁ : A₀ → A₁ → Prop}{a₀₁ : in-U~ a₀ a₁ A₀₁}(a₀₁ᴰ : in-U~ᴰ a₀ a₁ a₀₁)
    {B₀ : A₀ → Set}{b₀ : (x₀ : A₀) → in-U (B₀ x₀)}(b₀ᴰ : (x₀ : A₀) → in-Uᴰ (b₀ x₀))
      {B₀~ : ∀{x₀ x₁}(x₀₁ : A₀~ x₀ x₁) → B₀ x₀ → B₀ x₁ → Prop}
      {b₀~ : ∀{x₀ x₁}(x₀₁ : A₀~ x₀ x₁) → in-U~ (b₀ x₀) (b₀ x₁) (B₀~ x₀₁)}
      (b₀~ᴰ : ∀{x₀ x₁}(x₀₁ : A₀~ x₀ x₁) → in-U~ᴰ (b₀ x₀) (b₀ x₁) (b₀~ x₀₁))
    {B₁ : A₁ → Set}{b₁ : (x₁ : A₁) → in-U (B₁ x₁)}(b₁ᴰ : (x₁ : A₁) → in-Uᴰ (b₁ x₁))
      {B₁~ : ∀{x₀ x₁}(x₀₁ : A₁~ x₀ x₁) → B₁ x₀ → B₁ x₁ → Prop}
      {b₁~ : ∀{x₀ x₁}(x₀₁ : A₁~ x₀ x₁) → in-U~ (b₁ x₀) (b₁ x₁) (B₁~ x₀₁)}
      (b₁~ᴰ : ∀{x₀ x₁}(x₀₁ : A₁~ x₀ x₁) → in-U~ᴰ (b₁ x₀) (b₁ x₁) (b₁~ x₀₁))
    ...
    {b₀₁ : ∀{x₀ x₁}(x₀₁ : A₀₁ x₀ x₁) → in-U~ (b₀ x₀) (b₁ x₁) (B₀₁ x₀₁)}
    (b₀₁ᴰ : ∀{x₀ x₁}(x₀₁ : A₀₁ x₀ x₁) → in-U~ᴰ (b₀ x₀) (b₁ x₁) (b₀₁ x₀₁)) → 
    in-U~ᴰ (π a₀ a₀~ b₀ b₀~) (π a₁ a₁~ b₁ b₁~) (π~ {a₀ = a₀}{a₀~ = a₀~}{a₁ = a₁}{a₁~ = a₁~} a₀₁ {b₀ = b₀}{b₀~ = b₀~}{b₁ = b₁}{b₁~ = b₁~} b₀₁))
  where
  ...
  ind-in-U~' : {A₀ A₁ : Set} {A₀₁ : A₀ → A₁ → Prop} {a₀ : in-U A₀}{a₁ : in-U A₁}
             ...
             → in-U~ᴰ a₀ a₁ (a₀₁ₚ ,sp a₀₁ₜ)

  ind-in-U' boolₚ y = boolᴰ
  ind-in-U' (πₚ aₚ a~ₚ bₚ b~ₚ) w =
    πᴰ (ind-in-U' aₚ a)
       (ind-in-U~' a~ₚ a~)
  ...
  ind-in-U~' {a₀ = a₀} {a₁} bool~ₚ y =
    goal (to-≡ (proj₁p (inv-bool~ₜ-≡ y))) (to-≡ (proj₂p (inv-bool~ₜ-≡ y)))
    where
      goal : proj₁sp a₀ ≡ boolₚ -> proj₁sp a₁ ≡ boolₚ -> _
