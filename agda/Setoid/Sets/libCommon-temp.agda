...
get-π-data : {A : _} → in-U A → ⊤ ⊎ π-data
get-π-data = simple-just-U.ind-in-U (λ x → ⊤ ⊎ π-data) (inj₁ tt) λ {_} {a} _ {_} {a~} {_} {b} _ {_} {b~} →
  inj₂ (record { A = _ ; A~ = _ ; B = _ ; B~ = _ ; a = a ; a~ = a~ ; b = b ; b~ = b~ })

⊤' : Set₁
⊤' = {A : Set} → A → A

pj-π~-ty : ⊤ ⊎ π-data → ⊤ ⊎ π-data → Set₁
pj-π~-ty (inj₁ x) y = ⊤'
pj-π~-ty (inj₂ x) (inj₁ x₁) = ⊤'
pj-π~-ty (inj₂ record { A = A₀ ; A~ = A₀~ ; B = B₀ ; B~ = B₀~ ; a = a₀ ; b = b₀ })
         (inj₂ record { A = A₁ ; A~ = A₁~ ; B = B₁ ; B~ = B₁~ ; a = a₁ ; b = b₁ }) =
...
pj-π~ : {A₀ A₁ : _} {a₀ : in-U A₀} {a₁ : in-U A₁} {A~ : A₀ → A₁ → Prop}
      → in-U~ a₀ a₁ A~ → pj-π~-ty (get-π-data a₀) (get-π-data a₁)
pj-π~ {A₀}{A₁}{a₀}{a₁}{A~} = simple.ind-in-U~ (λ _ → ⊤) (λ {_} {_} {a₀} {a₁} _ → pj-π~-ty (get-π-data a₀) (get-π-data a₁))
...
pj~-ty : (p0 : ⊤ ⊎ π-data) (p1 : ⊤ ⊎ π-data) → pj-π~-ty p0 p1 → (A₀ A₁ : Set) → (A₀ → A₁ → Prop) → Set₁
pj~-ty (inj₁ x) y _ _ _ _ = ⊤'
pj~-ty (inj₂ y₁) (inj₁ x) _ _ _ _ = ⊤'
