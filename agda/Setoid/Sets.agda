...
cotr : ∀{A₀ A₁}{a₀ : in-U A₀}{a₁ : in-U A₁} →
  ((Â₀₁ : (A₀ ,Σ a₀) ~U (A₁ ,Σ a₁))(x₀ : A₀) → Σsp A₁ (El~ Â₀₁ x₀)) ×
  (∀{A₂}{a₂ : in-U A₂}{A₀₁ : A₀ → A₁ → Prop}{A₁₂ : A₁ → A₂ → Prop}(a₀₁ : in-U~ a₀ a₁ A₀₁)(a₁₂ : in-U~ a₁ a₂ A₁₂) → Σsp
    (Σ (A₀ → A₂ → Prop) (in-U~ a₀ a₂)) λ a₀₂' →
    ({x₀ : A₀}{x₁ : A₁}{x₂ : A₂} → El~ (tr (A₀₁ ,Σ a₀₁)) x₀ x₁ → El~ (tr (A₁₂ ,Σ a₁₂)) x₁ x₂ → El~ (tr a₀₂') x₀ x₂))
  
cotr {a₀ = bool}                {bool}                 =
  (λ _ x → x ,sp  refEl {_ ,Σ bool} x) ,Σ
  λ { {_}{bool} bool~ bool~ → (_ ,Σ bool~) ,sp λ { {tt}{tt}{tt} _ _ → ttp ; {ff}{ff}{ff} _ _ → ttp } }
cotr {a₀ = bool}                {π a a~ b b~}          = (λ w _ → ⊥pelim (withTrunc w λ ())) ,Σ λ ()
cotr {a₀ = π a a~ b b~}         {bool}                 = (λ w _ → ⊥pelim (withTrunc w λ ())) ,Σ λ ()
cotr {a₀ = π {A₀} a₀ a₀~ b₀ b₀~}{π {A₁} a₁ a₁~ b₁ b₁~} = (λ { w (f₀ ,sp f₀~) →
  ...
  λ { {_}{π {A₂} a₂ a₂~ {B₂} b₂ b₂~}(π~ {A₀₁ = A₀₁} a₀₁ {B₀₁ = B₀₁} b₀₁)(π~ {A₀₁ = A₁₂} a₁₂ {B₀₁ = B₁₂} b₁₂) → (_ ,Σ π~
    (proj₂ (proj₁sp (proj₂ cotr a₀₁ a₁₂)))
    {B₀₁ = λ {x₀}{x₂} x₀₂ → proj₁ (proj₁sp (proj₂ cotr
      (b₀₁ (fromEl~ a₀₁ (proj₂sp (proj₁ cotr (tr (_ ,Σ a₀₁)) x₀))))
      ...
      (b₁~ (fromEl~ a₁~ (proj₂sp (proj₂ cotr
         ...
         (proj₂sp (proj₂ cotr
           ...
           (proj₂sp (proj₁ cotr (tr (symU a₁₂)) x₂)))))) (b₁₂ (fromEl~ a₁₂ (symEl (proj₂ (symU a₁₂)) (proj₂sp (proj₁ cotr (tr (symU a₁₂)) x₂))))))))))}
    λ {x₀}{x₂} x₀₂ → proj₂ (proj₁sp (proj₂ cotr
      ...
      (b₁~ (fromEl~ a₁~ (proj₂sp (proj₂ cotr
         ...
         (proj₂sp (proj₂ cotr
           ...
           (proj₂ (symU a₁₂)))
           (toEl~ (proj₂ (proj₁sp (proj₂ cotr a₀₁ a₁₂))) x₀₂)
           (proj₂sp (proj₁ cotr (tr (symU a₁₂)) x₂)))))) (b₁₂ (fromEl~ a₁₂ (symEl (proj₂ (symU a₁₂)) (proj₂sp (proj₁ cotr (tr (symU a₁₂)) x₂))))))))))) ,sp
    {!!} }
...
{-
transUEl {a₀ = bool}{bool}{_}{bool} bool~ bool~ = (_ ,Σ bool~) ,sp λ { {tt}{tt}{tt} _ _ → ttp ; {ff}{ff}{ff} _ _ → ttp }
transUEl {a₀ = π {A₀} a₀ a₀~ {B₀} b₀ b₀~}{π {A₁} a₁ a₁~ {B₁} b₁ b₁~}
  {_}{π {A₂} a₂ a₂~ {B₂} b₂ b₂~}(π~ {A₀₁ = A₀₁} a₀₁ {B₀₁ = B₀₁} b₀₁)(π~ {A₀₁ = A₁₂} a₁₂ {B₀₁ = B₁₂} b₁₂) = (_ ,Σ
  π~ (proj₂ (transU a₀₁ a₁₂))
     ...
     λ {x₀}{x₂} x₀₂ → proj₂ (transU
       ...
       (b₁~ (fromEl~ a₁~ (transEl
          ...
          (transEl
            ...
            (proj₂ (symU a₁₂))
            (toEl~ (proj₂ (transU a₀₁ a₁₂)) x₀₂)
            (cohEl (tr (symU a₁₂)) x₂))))) (b₁₂ (fromEl~ a₁₂ (symEl (proj₂ (symU a₁₂)) (cohEl (tr (symU a₁₂)) x₂)))))))) ,sp
